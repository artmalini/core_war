...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/07/26 15:54:40 by amakhiny          #+#    #+#             */
/*   Updated: 2018/07/26 15:54:52 by amakhiny         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	vm_xor_5(t_vm *vm, t_cmd *cmd, int *arg, int x)
{
...
void	vm_xor_4(t_vm *vm, t_cmd *cmd, int *arg, int x)
{
	...
	else if (((0xFF & vm->arena[mdx(cmd->idx + 1)].acb)) == 116)
	{
		...
		arg[4] = 0;
		if (vm_v_cmd(arg[0] - 1, arg[0] - 1, arg[1] - 1))
			vm_rir_bit(vm, cmd, arg, 3);
		vm_next_step(vm, cmd, vm_pos_curs(vm, cmd));
	}
	vm_xor_5(vm, cmd, arg, x);
}

void	vm_xor_3(t_vm *vm, t_cmd *cmd, int *arg, int x)
{
	...
	else if (((0xFF & vm->arena[mdx(cmd->idx + 1)].acb)) == 164)
	{
		...
		arg[4] = 4;
		if (vm_v_cmd(arg[0] - 1, arg[0] - 1, arg[0] - 1))
			vm_ddr_bit(vm, cmd, arg, 3);
		vm_next_step(vm, cmd, vm_pos_curs(vm, cmd));
	}
	vm_xor_4(vm, cmd, arg, x);
}

void	vm_xor_2(t_vm *vm, t_cmd *cmd, int *arg, int x)
{
